...
- Possible threat detection for BSSIDs that appear to follow the scanner across
  accurate MGRS locations over time.
...
The current output is condensed to one row per `BSSID`. `MGRS Unique Count` counts unique non-empty MGRS locations for that BSSID, treating locations within the configured meter threshold as the same location.
